...
- (id)initWithTitle:(NSString *)title
{
    if (self = [super init]) {
        ...
        [_contentListView setTitle:@"重复新密码:"];
        ...
        sureBtn.frame = CGRectMake(0,160,self.frame.size.width/2,40);
        ...
        cancelBtn.frame = CGRectMake(self.frame.size.width/2,160,self.frame.size.width/2,40);
        [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
        [cancelBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [cancelBtn addTarget:self action:@selector(btnDown:) forControlEvents:UIControlEventTouchUpInside];
        cancelBtn.tag = 1001;
...
- (BOOL)check
{
    ...
    if (_numListView.textField.text.length==0) {
        [RemindView showViewWithTitle:@"请输入重置密码" location:MIDDLE];
        return NO;
    }
    if (_contentListView.textField.text.length == 0) {
        [RemindView showViewWithTitle:@"请重复一次重置密码" location:MIDDLE];
        return NO;
    }
    if (![_numListView.textField.text isEqualToString:_contentListView.textField.text]) {
        [RemindView showViewWithTitle:@"两次输入的密码不一致" location:MIDDLE];
        return NO;
    }
    return YES;
}

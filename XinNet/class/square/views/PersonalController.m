...
{
    CGFloat height = 40;
    CGFloat firstHeight = 80;   //第一列的高度
    UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(0,0,kWidth,firstHeight+height*4)];
    ...
    for (int i = 0 ; i < 5; i++) {
        ...
        if (i == 0) {
            line.frame = CGRectMake(0,firstHeight-1,kWidth,1);
        }else{
            line.frame = CGRectMake(0,firstHeight+height*i-1,kWidth,1);
        }
        line.backgroundColor = HexRGB(0xd5d5d5);
        [bgView addSubview:line];
    }
    
    _iconView = [[ListView alloc] initWithFrame:CGRectMake(10,0,kWidth-10,firstHeight)];
    [_iconView setTitle:@"头像"];
    [bgView addSubview:_iconView];
    
    _iconImage = [[TJImageView alloc] initWithFrame:CGRectMake(_iconView.frame.size.width-10-60,(firstHeight-60)/2,60, 60)];
    _iconImage.image = [UIImage imageNamed:@"l"];
    _iconImage.delegate = self;
    [_iconView addSubview:_iconImage];
    
    
    _accountView = [[ListView alloc] initWithFrame:CGRectMake(10,firstHeight,kWidth-10,height)];
    [_accountView setTitle:@"账号"];
    _accountView.detailLabel.text = @"xxxxxxx";
    [bgView addSubview:_accountView];

    _nickNameView = [[ListView alloc] initWithFrame:CGRectMake(10,firstHeight+height,kWidth-10,height)];
    [_nickNameView setTitle:@"昵称"];
    _nickNameView.detailLabel.text = @"xxxxxxxxx";
    [bgView addSubview:_nickNameView];

    _emailView = [[ListView alloc] initWithFrame:CGRectMake(10,firstHeight+height*2,kWidth-10,height)];
    [_emailView setTitle:@"邮箱"];
    _emailView.detailLabel.text = @"xxxxxxxxx";
    [bgView addSubview:_emailView];

    _phoneView = [[ListView alloc] initWithFrame:CGRectMake(10,firstHeight+height*3,kWidth-10,height)];

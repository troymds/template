...
- (id)initWithTitle:(NSString *)title Content:(NSString *)content description:(NSString *)description url:(NSString *)url delegate:(id)delegate
{
    if (self = [super init]) {
        ...
        [_container setIPhoneContainerWithViewController:delegate];
        _shareList = [ShareSDK getShareListWithType:
                              ShareTypeSinaWeibo,
                              ShareTypeQQ,
                              ShareTypeQQSpace,
                              ShareTypeWeixiSession,
        ...
        _publishContent = [ShareSDK content:_content
                             defaultContent:@""
                                      image:nil
                                      title:_title
                                        url:_url
        ...
        _shareOptions = [ShareSDK defaultShareOptionsWithTitle:@"" oneKeyShareList:_shareList cameraButtonHidden:YES mentionButtonHidden:NO topicButtonHidden:NO qqButtonHidden:NO wxSessionButtonHidden:NO wxTimelineButtonHidden:NO
        showKeyboardOnAppear:NO shareViewDelegate:nil friendsViewDelegate:nil picViewerViewDelegate:nil];
...
- (void)showShareView
{
    ...
    [ShareSDK showShareActionSheet:_container
                                     ...
                                     id<ICMErrorInfo> error, BOOL end)
     {
         if (state == SSPublishContentStateSuccess)
         {
             UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"分享成功!" message:nil delegate:_delegate cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
             [alertView show];
         }
         else if (state == SSPublishContentStateFail)
         {
             if ([error errorCode] == -22003) {
                 if ([error errorCode] == -22003) {
                     UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"分享失败" message:@"尚未安装微信,请安装后重试！" delegate:_delegate cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                     [alertView show];
                 }
             }else if ([error errorCode] == -24002){
                 UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"分享失败" message:@"尚未安装QQ,请安装后重试！" delegate:_delegate cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                 [alertView show];
             }else if ([error errorCode] == -6004){
                 UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"分享失败" message:@"尚未安装QQ或者QQ空间客户端，请安装后重试！" delegate:_delegate cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                 [alertView show];
             }
         }
     }];
